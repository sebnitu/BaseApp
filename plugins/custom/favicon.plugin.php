...
class Favicon {
	 ...
	 * Favicon Processing
	 */
	function process($site_url, $if_modified_since = 0) {       
	...
	/**
	 * Check that a Favicon exists
	 */
	function is_ico_exists() {
		if ($this->ico_exists == 'not checked') {
			$result = $this->check_url($this->ico_url);
			$this->ico_exists = ($result) ? true : false;
		}
	...
	/*-------------------------------------------    
		Saving and Deleting Favicon
	---------------------------------------------*/
	
	/**
	 * Save File
	 * Unique case file uploading/saving
	 */
	function save_favicon($filename) {
		...
		// Open connections and start cURL session
		$ch = curl_init($this->ico_url);
